...
def run_cross_validation(all_paths, all_speeds, stats, dataset_name="default"):
    """
    ...
    checkpoint_dir = Config.get_dataset_checkpoint_dir(dataset_name)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(paths_np, all_speeds)):
        ...
        # Callbacks
        checkpoint_path = os.path.join(checkpoint_dir, f"fold_{fold+1}_best.keras")
        callbacks = [
        ...
        history = model.fit(
            ...
            callbacks=callbacks,
            verbose=1 # Show progress bar and batch updates
        )
